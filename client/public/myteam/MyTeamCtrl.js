(function() {
  ...
  _loadD3Tree(convertedTree);
  		...
  		function updateTree(baId) {
  			clearTree(false);
  			var paths = _searchTree(null, baId, []);
  			if (typeof(paths) !== "undefined") {
  				_openPaths(paths, baId);
  			} else {
  				var msg = getModel("i18n").getResourceBundle().getText("Tree.searchNotFound", baId);
  				MessageToast.show(msg);
  			}
  		}

  	 function clearTree(clearCall) {
  			_clearNodes();
  			if (clearCall) {
  				_update();
  			}
  ...
  function _loadD3Tree (treeData) {
		 ...
		 var baseSvg = d3.select("#tree-container").append("svg")
			...
			function getInfo(d) {
			 ...
			 function _clearNodes(childnodes) {
				if (childnodes === null || childnodes === undefined) {
					childnodes = root.children ? root.children : root._children;
					root.class = "";
				}
				var that = this;
				if (childnodes != null && childnodes != undefined) {
					childnodes.forEach(function(d) {
						...
						if (d.children || d._children) {
							...
							_clearNodes(children);
						}
					});
				}
			};

		 function _searchTree(searchNode, baId, path) {
				if (searchNode === null) {
					searchNode = root;
				}
				if (searchNode.name === baId) { //if search is found return, add the object to the path and return it
					path.push(searchNode);
					return path;
				} else if (searchNode.children || searchNode._children) { //if children are collapsed d3 object will have them instantiated as _children
					var children = (searchNode.children) ? searchNode.children : searchNode._children;
					for (var i = 0; i < children.length; i++) {
						path.push(searchNode); // we assume this path is the right one
						var found = _searchTree(children[i], baId, path);
			  ...
			  function _openPaths(paths, baId) {
				var searchFlag = false;
				for (var i = 0; i < paths.length; i++) {
					if (paths[i].id !== "1") { //i.e. not root
						paths[i].class = "found";
						if (paths[i].name === baId) {
							searchFlag = true;
						}
						if (paths[i]._children && !searchFlag) { //if children are hidden: open them, otherwise: don't do anything
